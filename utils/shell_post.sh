...
cp grid_private.shp grid.shp
cp grid_private.prj grid.prj
cp grid_private.shx grid.shx
cp grid_public.dbf grid.dbf

## shapefile to geoJSON and transform to wgs84
ogr2ogr -f GeoJSON -t_srs crs:84 grid.geojson grid.shp 

## dbf to csv because foreign::read.dbf() does not like a url pathname
ogr2ogr -f CSV grid.csv grid_public.dbf
...
ogr2ogr -f GeoJSON -t_srs crs:84 -lco WRITE_BBOX=YES aoi.geojson aoi.shp 
